<template>
  <div class="block_3_bill">
      ...
      <div class="cart_product" v-else>
        <div class="list_item">
          <div class="item" v-for="(item, index) in list_product" :key="index" @click.stop.prevent="handleAdjust(item)">
...
export default {
  ...
  computed: {
    list_product: function(){
      let vm = this;
      vm.$parent.$parent.data.map((item) => {
        if(!vm.arr_product.includes(item)){
          vm.arr_product.push(item)
        }
      });
      return vm.arr_product;
    }
  }
}
</script>
